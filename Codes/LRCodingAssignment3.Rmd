...
# 4 levels, reference level healthy
```
...
# "underweight" is not linear because p value is insignificant{highest}. p is directly proportional to feature. Null hypothesis
# **How worse is the disease progression in unhealthy people compared to the healthy ones?**
# calculate yhat. 103.967 units higher
...
# refer written notes for further calculation
...
#  lower p value {significant}. S1, S5, S6
...
summary(model)
# find the significance and then change model
model = lm(data = df, Y~ BMILEVEL +S1 + S5 + S6 )
summary(model)
model1 = lm(data = df, Y ~ (BMILEVEL == "underweight") +S1 + S5 + S6)
summary(model1)

model2 = lm(data = df, Y ~ (BMILEVEL == "unhealthy") +S1 +  S5 + S6)
summary(model2)

model3 = lm(data = df, Y ~ (BMILEVEL == "healthy") +S1 +  S5 + S6)
summary(model3)

model4 = lm(data = df, Y ~ (BMILEVEL == "overweight") +S1 + S5 + S6)
summary(model4)
...
```{r}
# Fit a linear model for predicting disease progression using BMI, age, BP, and gender. How accurate is the model? 39%
# **According to the model, which gender has a worse disease progression? Explain why.**
# yhat(male) - yhat(female) = -10.16 males have disease progression 10.16% less than female.
...
# interaction model is used when we have continuous variables. 
